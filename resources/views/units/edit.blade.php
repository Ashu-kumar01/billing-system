<x-app-layout>
    <x-page-header title="Edit Unit" subtitle="Update unit details." />
        ...
        <form method="POST" action="{{ route('units.update', $unit) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    ...
                    <x-text-input id="name" name="name" type="text" class="mt-1" :value="old('name', $unit->name)" required autofocus />
                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="short_name" value="Short Name" />
                    <x-text-input id="short_name" name="short_name" type="text" class="mt-1" :value="old('short_name', $unit->short_name)" maxlength="20" required />
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="4" class="form-textarea mt-1">{{ old('description', $unit->description) }}</textarea>
                <x-input-error :messages="$errors->get('description')" class="mt-2" />
            </div>

            <div class="flex items-center gap-3">
                <label for="status" class="relative inline-flex cursor-pointer items-center">
                    <input type="checkbox" id="status" name="status" value="1" class="peer sr-only" {{ old('status', $unit->status) ? 'checked' : '' }}>
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-border">
                <a href="{{ route('units.index') }}" class="btn-secondary">Cancel</a>
                <x-primary-button>
